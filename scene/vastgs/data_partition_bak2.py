...
class ProgressiveDataPartitioning:
    ...
    #     return
    def draw_partition(self, partition_list):
        # 使用matplotlib的tab20调色盘生成足够多且区分度高的颜色
        cmap = plt.get_cmap('tab20')
        colors = [cmap(i) for i in range(cmap.N)]  # 获取调色盘中的所有颜色
        ...
        for i, partition in enumerate(partition_list):
            ...
            # 创建原始矩形框
            rect_ori = patches.Rectangle(
                (x_min, z_min), x_max - x_min, z_max - z_min,
                linewidth=1, edgecolor=color, facecolor='none'
            )

            # # 创建扩展矩形框
            # rect_ext = patches.Rectangle(
            #     (ex_x_min, ex_z_min), ex_x_max - ex_x_min, ex_z_max - ex_z_min,
            #     linewidth=1, edgecolor=color, facecolor='none'
            # )
            ...
            # 添加文字标注，颜色与矩形框颜色一致
            self.ax.text(
                x=rect_ori.get_x(), y=rect_ori.get_y(),
                s=f"{partition.partition_id}", color=color, fontsize=12
            )

        # 保存图片
        self.fig.savefig(os.path.join(self.model_path, 'regions.png'), dpi=200)
        return
    ...
    def run_DataPartition(self, train_cameras):
        if not os.path.exists(self.save_partition_data_dir):
            ...
            # 根据粗略划分的结果，进行调整细化
            # partition_dict, refined_ori_bbox = self.refine_ori_bbox_visualHull(partition_dict)
            ...
            #     # FIXME: 这里不能只是很简单的+ '.jpg'啊,需要判断一下!!!
            #     # image_name_list = [camera_info[i].camera.image_name + '.jpg' for i in range(len(camera_info))]
            #     image_name_list = [camera_info[i].camera.image_name + '.png' for i in range(len(camera_info))]
            #     txt_file = f"{self.partition_ori_dir}/{partition.partition_id}_camera.txt"
    ...
    def Camera_position_based_region_division(self, train_cameras):
        ...
        for idx, camera in enumerate(train_cameras):
            ...
            rotation = np.array(camera.camera_rotation.cpu())
            camera_centers.append(pose)
            CameraPose_list.append(
                CameraPose(camera=camera, pose=pose, rotation=rotation))  # 世界坐标系下相机的中心坐标
        ...
        for i in range(m):  # 按照x轴将所有相机分成m部分
            ...
            if i != m - 1:
                m_partition_dict[str(i + 1)].update({"x_mid_camera": sorted_CameraPose_by_x_list[
                    (i + 1) * num_of_camera_per_m_partition - 1]})  # 将左边块的相机作为无缝衔接的边界
            else:
    ...
    def Visibility_based_camera_selection(self, partition_list):
        ...
        for idx, partition_i in enumerate(partition_list):  # 第i个partition
            ...
            storePly(os.path.join(self.partition_visible_dir, f"{partition_id_i}_visible.ply"), new_points,
                     new_colors * 255)  # 保存可见性选择后每个partition的点云
    ...
    def refine_ori_bbox_visualHull(self, partition_dict):
        ...
        for partition_idx, camera_list in partition_dict.items():
            ...
            print("Partition", partition_idx, "ori_camera_bbox", ori_camera_bbox)
            ...
            # 创建 CameraPartition 对象并添加到 partition_list
            partition_list.append(CameraPartition(partition_id=partition_idx,
                                                  cameras=camera_list,
                                                  point_cloud=BasicPointCloud(points, colors, normals),
                                                  ori_camera_bbox=ori_camera_bbox,
                                                  extend_camera_bbox=None,  # 没有拓展，所以这里是 None
                                                  extend_rate=None,  # 没有拓展
                                                  ori_point_bbox=self.get_point_range(points),
                                                  extend_point_bbox=None))  # 没有拓展的点云范围
